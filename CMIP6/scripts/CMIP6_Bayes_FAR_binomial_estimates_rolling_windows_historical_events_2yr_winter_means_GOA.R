...
## probabilities for given 2-yr rolling mean winter sst anomalies
## this is a side product for GOA pollock project
...
outcomes <- left_join(outcomes, weights) %>%
    select(model, model_weight, period, region, ersst.year, winter.anomaly.2yr, winter.2yr.events)

# drop NAs due to smoothing
drop <- is.na(outcomes$winter.anomaly.2yr)
...
prop <- dt[ , .(count = sum(winter.2yr.events, na.rm = T),
                N = .N,
                winter.anomaly.2yr = unique(winter.anomaly.2yr),
...
form <-  bf(count | trials(N) + weights(model_weight, scale = TRUE) ~
            period + s(winter.anomaly.2yr, by = period, k = 6) +
...
saveRDS(far_brms2, paste("./CMIP6/brms_output/", regions[i], "_2yr_mean_winter_sst_rolling_window_binomial2.rds", sep = ""))
