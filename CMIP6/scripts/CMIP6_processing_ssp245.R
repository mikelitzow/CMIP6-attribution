...
# for ssp245 runs to provide alternate estimates of warming rate
...
for(i in 1:length(files.new)){ # start i loop (each CMIP6 model)
  ...
  path <- paste("./CMIP6/CMIP6_outputs/1850-2099_runs/ssp245/", files.new[i], sep="")
...
# save summaries
write.csv(CMIP6.sst.time.series, "./CMIP6/summaries/CMIP6.sst.time.series.ssp245.csv", row.names = F)
write.csv(CMIP6.anomaly.time.series, "./CMIP6/summaries/CMIP6.anomaly.time.series.ssp245.csv", row.names = F)
