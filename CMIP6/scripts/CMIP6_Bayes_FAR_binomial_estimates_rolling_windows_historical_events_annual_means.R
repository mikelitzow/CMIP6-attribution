...
prop <- dt[ , .(count = sum(annual.1yr.events, na.rm = T),
                N = .N,
...
for(i in 1:length(regions)){
    ...
    print(paste("Region #", i, ": ", regions[i], sep = ""))
    print("")
    check_hmc_diagnostics(far_brms2$fit)
    print("")
    print(neff_lowest(far_brms2$fit))
    print("")
    print(rhat_highest(far_brms2$fit))
}

# North Pacific model has one divergence - refit with higher adapt_delta
i <- 1
preindustrial <- read.csv(paste("./CMIP6/summaries/", regions[i], "_preindustrial_outcomes.csv", sep = ""))
