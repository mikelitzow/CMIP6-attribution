...
warming.rate <- sst.245

# now get anomaly wrt 1850:1949
for(j in 2:ncol(sst.245)){
  
  warming.rate[,j] <- sst.245[,j] - colMeans(sst.245[1:100,])[j]
...
# save
write.csv(n.pac.warming.timing, "./CMIP6/summaries/loess_smoothed_model_warming_rates_ssp245.csv", row.names = F)
...
ggsave("./CMIP6/figs/N_Pac_warming_rate_by_model_ssp245.png", width = 6, height = 4, units = 'in')
...
# save timing
write.csv(timing, "./CMIP6/summaries/model.north.pacific.warming.timing.ssp245.csv")
write.csv(n.pac.obs.timing, "./CMIP6/summaries/ersst.observed.warming.csv")
...
predict.timing <- predict.timing %>% 
  pivot_longer(cols = c(-year, -ersst))
...
# save 
write.csv(model.warming.evaluation, "./CMIP6/summaries/N_Pac_warming_model_weights_ssp245.csv", row.names = F)
